...
if __name__ == "__main__":
    ...
    source = "R3.jpg"
    target_item = "orange"
    results = model.predict(source, conf=0.25, iou=0.5)
    result = results[0]
    exclude = ["potted plant"]
    img_width = result.orig_img.shape[1] if hasattr(result, "orig_img") else None

    pos, layer = compute_placement(result, target_item, exclude_classes=exclude, img_width=img_width)

    if pos:
        print(f"推荐放置 {target_item}: {pos} (层 {layer})")
        draw_point(source, pos)
    else:
        print("未找到合适放置点")
